...
variables_for_l531_a <- mcf_data%>%look_for(keyword_label)
...
for (i in 1:length(keyword_label) ){
  mcf_data[,var_df[i,]][mcf_data[,var_df[i,]]==1]<-0
  mcf_data[,var_df[i,]][mcf_data[,var_df[i,]]==2]<-1
  mcf_data[,var_df[i,]][mcf_data[,var_df[i,]]==3]<-2
  mcf_data[,var_df[i,]][mcf_data[,var_df[i,]]==4]<-3
  mcf_data[,var_df[i,]][mcf_data[,var_df[i,]]==5]<-4
}

# change the values from 1-3 to 0-2 
keyword_label_b<-c("D15",	"D17",	"D19",	"D21",	"D23",	"D25",	"D27",	"D29",	"D31",	"D33",	"D35",	"D37")
variables_for_l531_b <- mcf_data%>%look_for(keyword_label_b)
...
for (i in 1:length(keyword_label_b) ){
  mcf_data[,var_df_b[i,]][mcf_data[,var_df_b[i,]]==1]<-0
  mcf_data[,var_df_b[i,]][mcf_data[,var_df_b[i,]]==2]<-1
  mcf_data[,var_df_b[i,]][mcf_data[,var_df_b[i,]]==3]<-2
}
...
data1<-as.data.frame(mcf_data)%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life,weights,gender, age_group, geo_entity,pwd,refuge,stratum, main_activity, sector_choices,c(sector_choices_name_1:youth_improv_work_1)) #add weights column 
data2<-as.data.frame(mcf_data)%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life,weights,gender, age_group, geo_entity,pwd,refuge,stratum, main_activity, sector_choices,c(sector_choices_name_2:youth_improv_work_2))
data3<-as.data.frame(mcf_data)%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life,weights,gender, age_group, geo_entity,pwd,refuge,stratum, main_activity, sector_choices,c(sector_choices_name_3:youth_improv_work_3))
data4<-as.data.frame(mcf_data)%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life,weights,gender, age_group, geo_entity,pwd,refuge,stratum, main_activity, sector_choices,c(sector_choices_name_4:youth_improv_work_4))
...
combination2<-combination%>%
  dplyr::select(uniqueid,gender,DFWIA1,ability_score,expectation_score,perc_quality_life, geo_entity, refuge, pwd, age_group, stratum,ease_access_digni_1 ,employing_youth_1,
...
combination5<-combination%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life, gender, geo_entity, refuge, pwd, age_group, stratum,geo_entity, youth_contrib_econ_1,youth_contrib_innov_1,
...
combination4<-mcf_data%>%
  dplyr::select(uniqueid,DFWIA1,ability_score,expectation_score,perc_quality_life,gender, geo_entity, refuge, pwd, age_group, stratum, get_work,workplaces_val,
...
combined_agency <- combination2%>%
  dplyr::left_join(combination5)%>%
  dplyr::left_join(combination4)

n_occur <- data.frame(table(combined_agency$uniqueid))

combined_agency_indic <- combined_agency %>%
  select(c("uniqueid","DFWIA1","ability_score","expectation_score","perc_quality_life",
            "gender", "geo_entity", "refuge", "pwd", "age_group", "stratum",
            "aspirations","growth","access","weights"))

filtered_combined_agency_indic<-combined_agency_indic%>%
  dplyr::group_by(uniqueid,gender,geo_entity,refuge,pwd,age_group,stratum,
                  weights,DFWIA1,ability_score,expectation_score,perc_quality_life)%>%
  dplyr::top_n(1,aspirations)%>%
  dplyr::top_n(1,growth)%>%
  dplyr::top_n(1,access)%>%
  dplyr::distinct()
