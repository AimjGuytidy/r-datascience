...
#mrker1n2 
two_markers_total<-characterize(mcf_data)%>%
  group_by(mkr1n2)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
two_markers_gender<-as.data.frame(characterize(mcf_data))%>%
  group_by(mkr1n2, gender)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =gender,  values_from = total)%>%
  as.data.frame()%>%
  move_columns(`b. Female`, .before=`a. Male`)

two_markers_refugee<-characterize(mcf_data)%>%
  group_by(mkr1n2, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
two_markers_pwd<-characterize(mcf_data)%>%
  group_by(mkr1n2,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
two_markers_geo<-characterize(mcf_data)%>%
  group_by(mkr1n2,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
two_markers_age<-characterize(mcf_data)%>%
  group_by(mkr1n2,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()

two_markers_stratum<-characterize(mcf_data)%>%
  group_by(mkr1n2,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()

two_markers_final<-two_markers_total%>%
  left_join(two_markers_gender)%>%
  left_join(two_markers_refugee)%>% 
  left_join(two_markers_pwd)%>%
  left_join(two_markers_geo)%>%
  left_join(two_markers_age)%>%
  left_join(two_markers_stratum)

write.xlsx(two_markers_final, 'two_markers_final.xlsx',overwrite=TRUE)

#ISIC
two_markers_total<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
two_markers_gender<-as.data.frame(characterize(mcf_data))%>%
  group_by(mkr1n2,main_activity, gender)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =gender,  values_from = total)%>%
  as.data.frame()%>%
  move_columns(`b. Female`, .before=`a. Male`)
...
two_markers_refugee<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
two_markers_pwd<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
two_markers_geo<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
two_markers_age<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()

two_markers_stratum<-characterize(mcf_data)%>%
  group_by(mkr1n2,main_activity,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()
...
#DFW2 
DFW2_total<-characterize(mcf_data)%>%
  group_by(DFW2)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
DFW2_gender<-as.data.frame(characterize(mcf_data))%>%
  group_by(DFW2, gender)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =gender,  values_from = total)%>%
  as.data.frame()%>%
  move_columns(`b. Female`, .before=`a. Male`)

DFW2_refugee<-characterize(mcf_data)%>%
  group_by(DFW2, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
DFW2_pwd<-characterize(mcf_data)%>%
  group_by(DFW2,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
DFW2_geo<-characterize(mcf_data)%>%
  group_by(DFW2,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
DFW2_age<-characterize(mcf_data)%>%
  group_by(DFW2,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()
DFW2_stratum<-characterize(mcf_data)%>%
  group_by(DFW2,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()

DFW2_final<-DFW2_total%>%
  left_join(DFW2_gender)%>%
  left_join(DFW2_refugee)%>% 
  left_join(DFW2_pwd)%>%
  left_join(DFW2_geo)%>%
  left_join(DFW2_age)%>%
  left_join(DFW2_stratum)

write.xlsx(DFW2_final, 'DFW2_final.xlsx',overwrite=TRUE)

#ISIC 
DFW2_total<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
DFW2_gender<-as.data.frame(characterize(mcf_data))%>%
  group_by(DFW2,main_activity, gender)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =gender,  values_from = total)%>%
  as.data.frame()%>%
  move_columns(`b. Female`, .before=`a. Male`)
...
DFW2_refugee<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
DFW2_pwd<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
DFW2_geo<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
DFW2_age<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()
DFW2_stratum<-characterize(mcf_data)%>%
  group_by(DFW2,main_activity,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()
...
#DFWIA1
DFWIA1_total<-characterize(mcf_data)%>%
  group_by(DFWIA1)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
DFWIA1_gender<-as.data.frame(characterize(mcf_data))%>%
...
DFWIA1_refugee<-characterize(mcf_data)%>%
  group_by(DFWIA1, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
DFWIA1_pwd<-characterize(mcf_data)%>%
  group_by(DFWIA1,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
DFWIA1_geo<-characterize(mcf_data)%>%
  group_by(DFWIA1,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
DFWIA1_age<-characterize(mcf_data)%>%
  group_by(DFWIA1,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()

DFWIA1_stratum<-characterize(mcf_data)%>%
  group_by(DFWIA1,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()

DFWIA1_final<-DFWIA1_total%>%
  left_join(DFWIA1_gender)%>%
  left_join(DFWIA1_refugee)%>% 
  left_join(DFWIA1_pwd)%>%
  left_join(DFWIA1_geo)%>%
  left_join(DFWIA1_age)%>%
  left_join(DFWIA1_stratum)

write.xlsx(DFWIA1_final, 'DFWIA1_final.xlsx',overwrite=TRUE)

#ISIC
DFWIA1_total<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity)%>%
  dplyr::summarize(total=sum(weights))%>%
  as.data.frame()
DFWIA1_gender<-as.data.frame(characterize(mcf_data))%>%
  group_by(DFWIA1,main_activity, gender)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =gender,  values_from = total)%>%
  as.data.frame()%>%
  move_columns(`b. Female`, .before=`a. Male`)
...
DFWIA1_refugee<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity, refuge)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =refuge,  values_from = total)%>%
  select(-c(`a. Non refuge`))%>%
  as.data.frame()
DFWIA1_pwd<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity,pwd)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =pwd,  values_from = total)%>%
  select(-c(No))%>%
  as.data.frame()
DFWIA1_geo<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity,geo_entity)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =geo_entity,  values_from = total)%>%
  as.data.frame()
DFWIA1_age<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity,age_group)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =age_group,  values_from = total)%>%
  as.data.frame()

DFWIA1_stratum<-characterize(mcf_data)%>%
  group_by(DFWIA1,main_activity,stratum)%>%
  dplyr::summarize(total=sum(weights))%>%
  pivot_wider(names_from =stratum,  values_from = total)%>%
  as.data.frame()
