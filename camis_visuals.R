...
df <-
  read_dta("03_clean/final_dataset/Teacher_Headteacher_Survey_Round_clean_final.dta")
...
temp["camis_2_1_17"] <-
  "17. Lack of infranstructures such as electricity"
temp <- gsub('[0-9]+', '', temp)
temp <- gsub('\\.', '', temp)
df_camis_long <- df_camis_2 %>%
  rename(all_of(setNames(names(temp), temp))) %>%
  pivot_longer(cols = -caseid,
               names_to = "challenge",
               values_to = "status")
df_camis_long <-
  filter(df_camis_long,
         challenge != "C Did you face any challenges while entering data on CA-MIS?")
...
ggplot(data = df_camis_text, aes(x = challenge, y = total)) +
