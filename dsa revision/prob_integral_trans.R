...
par(mfrow=c(2,2))
hist(qexp(draws),freq = F)
lines(density(qexp(draws),na.rm = T),col = "green")
abline(v = mean(qexp(draws),na.rm = T))

hist(temp,freq = F)
lines(density(temp,na.rm = T),col = "green")
abline(v = mean(temp,na.rm = T))
